...
if AUTH_TYPE == 'auth0':
    ...
    def perform_logout():
        ...
        logout_url = f"https://{st.secrets['AUTH0_DOMAIN']}/v2/logout?client_id={st.secrets['AUTH0_CLIENT_ID']}&returnTo={st.secrets['AUTH0_CALLBACK_URL']}"
        st.markdown(f'<meta http-equiv="refresh" content="0; url={logout_url}">', unsafe_allow_html=True)
else:
    auth_config = {'client_id': st.secrets['GOOGLE_CLIENT_ID'], 'client_secret': st.secrets['GOOGLE_CLIENT_SECRET'], 'redirect_uri': st.secrets['GOOGLE_REDIRECT_URI'], 'allow_insecure_http': True}
