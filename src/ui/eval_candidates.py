...
def _load_ai_chats(count: int):
    return get_client().query('AI_chats', order_by='createdAt', direction='DESCENDING', limit=count)
...
def _render_chat_row(chat):
    ...
    if cols[3].button('Details', key=f"chat_details_{chat['id']}"):
        ...
        st.rerun()
    if 'chat_details' in st.session_state and chat['id'] in st.session_state.chat_details:
...
def _render_eval_row(ev):
    ...
    if cols[2].button('Details', key=f'ev_details_{ev.id}'):
        ...
        st.rerun()
    if 'eval_details' in st.session_state and ev.id in st.session_state.eval_details:
